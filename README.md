# Desafios de Programação

Repositório com projetos e desafios de programação em JavaScript. Cada projeto é uma aplicação web funcional com HTML, CSS e JavaScript.

---

## 📋 Projetos

### 001 - Calculadora de IMC

**Descrição:** Aplicação que calcula o Índice de Massa Corporal (IMC) e fornece uma classificação de peso baseada nos critérios da OMS.

**Funcionalidades:**

- Entrada de nome, peso (kg) e altura (m)
- Cálculo automático do IMC
- Classificação do resultado (Abaixo do peso, Peso normal, Sobrepeso, Obesidade grau 1/2/3)
- Validação de campos obrigatórios

**Tecnologias:** HTML5, CSS3, JavaScript (vanilla)

**Localização:** `001 imc/`

---

### 002 - Calculadora

**Descrição:** Calculadora web completa com interface inspirada em calculadoras clássicas, suportando as operações matemáticas básicas.

**Funcionalidades:**

- Operações: adição, subtração, multiplicação e divisão
- Botões de limpeza (CE/C) e backspace
- Suporte a números decimais
- Alternância de sinal (±)
- Display em tempo real

**Tecnologias:** HTML5, CSS3, JavaScript (vanilla)

**Localização:** `002 calculadora/`

---

### 003 - Conversor de Temperatura

**Descrição:** Aplicação que converte temperaturas entre diferentes escalas: Celsius, Fahrenheit e Kelvin.

**Funcionalidades:**
- Conversão entre Celsius, Fahrenheit e Kelvin
- Seleção flexível de unidades de origem e destino
- Validação de entrada de valores numéricos
- Resultado com precisão de 2 casas decimais

**Tecnologias:** HTML5, CSS3, JavaScript (vanilla)

**Localização:** `003 Conversor de Temperatura/`

---

## 🚀 Como Usar

1. Clone ou baixe o repositório
2. Navegue até a pasta do projeto desejado
3. Abra o arquivo `index.html` em seu navegador

Exemplo:

```bash
cd 001\ imc
# Abra index.html no navegador
```

---

## 📁 Estrutura de Projeto

Cada projeto segue a mesma estrutura:

```
NXX nome-do-projeto/
├── index.html      # Estrutura HTML
├── index.js        # Lógica JavaScript
└── style.css       # Estilos CSS
```

---

## 🔄 Novos Projetos

Para adicionar um novo projeto:

1. Crie uma nova pasta com o padrão `NXX nome-do-projeto/` (ex: `003 conversor-temperatura/`)
2. Adicione os arquivos: `index.html`, `index.js` e `style.css`
3. Implemente a lógica desejada
4. Atualize este README com a descrição do novo projeto

---

## 📝 Notas

- Todos os projetos utilizam JavaScript vanilla (sem frameworks)
- Interface responsiva e compatível com navegadores modernos
- Código bem comentado e fácil de entender

---

## 👨‍💻 Desenvolvedor

Projetos de desafios e aprendizado em desenvolvimento web.

---